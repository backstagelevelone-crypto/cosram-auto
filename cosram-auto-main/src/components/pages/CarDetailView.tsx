...
function PricePanel({
  ...
  /**
   * Eveniment Lead pentru noul Pixel Meta.
   *
   * Pixelul nou este instalat global în layout.tsx:
   * 1303394386182780
   */
  const trackLead = (tipLead: string) => {
    if (typeof window !== "undefined" && (window as any).fbq) {
      (window as any).fbq("track", "Lead", {
        content_name: displayName,
        content_category:
          (car as any).category ||
          (car as any).caroserie ||
          "Auto",
  ...
  /**
   * Păstrăm și evenimentul Contact existent.
   */
  ...
  const handleSubmit = () => {
    ...
    /**
     * IMPORTANT:
     * La apăsarea butonului WhatsApp,
     * Meta primește acum Lead.
     */
    trackLead("WhatsApp_Form");

    /**
     * Păstrăm și Contact pentru continuitate.
     */
...
Aștept să fiu contactat pentru mai multe detalii.`;
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-[rgba(0,0,0,0.08)] bg-white p-3 shadow-[0_8px_30px_rgba(0,0,0,0.06)]",
        className
      )}
    >
      ...
      <p className="mt-3 font-[family-name:var(--font-outfit)] text-sm text-[#6B6B6B]">
        ...
        (car as any).rataLunara != null ? (
          <>
            Rată de la{" "}
            <span className="font-semibold text-[#111111]">
              {car.monthlyPrice || (car as any).rataLunara}€/lună
            </span>
          </>
        ) : (
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-1.5 border-t border-b border-[rgba(0,0,0,0.06)] py-1">
        ...
        <div className="flex flex-wrap gap-2 text-[11px] font-bold text-[#111111]">
          <span className="rounded bg-[#F2F2F7] px-2 py-0.5 text-[#1a5632]">
            BT Direct
          </span>

          <span className="rounded bg-[#F2F2F7] px-2 py-0.5 text-[#FF5A00]">
            tbi bank
          </span>

          <span className="rounded bg-[#F2F2F7] px-2 py-0.5 text-[#4A90E2]">
            cosram
          </span>
        </div>
      </div>
...
function buildSpecs(
  ...
  const kmRulaj =
    c.kilometraj != null
      ? `${formatLocaleNumber(c.kilometraj)} km`
      : car.km != null
  ...
  const candidates: {
    ...
    },
    {
      icon: Fuel,
      label: "Combustibil",
      value: c.combustibil || car.fuel,
    },
    {
      icon: Settings2,
      label: "Cutie viteze",
      value: c.cutieViteze || car.transmission,
    },
    {
      icon: Cog,
      label: "Motor",
      value: c.motor || car.engine,
    },
    {
      icon: Zap,
      label: "Putere",
    ...
    },
    {
      icon: CarFront,
      label: "Caroserie",
      value: c.caroserie || car.category,
    },
    {
      icon: Palette,
      label: "Culoare",
      value: c.culoare || car.color,
    },
    {
      icon: DoorOpen,
      label: "Nr. uși",
    ...
    },
    {
      icon: CarFront,
      label: "Tracțiune",
      value: c.tractiune || car.drive,
    },
    {
      icon: ShieldCheck,
      label: "Inspecție tehnică",
      value: c.inspectieTehnica || car.itp,
    },
  ];
...
export default function CarDetailView({
  ...
  const images = normalizeCarImages(
    car.images || (car as any).galerie
  );
  ...
  /**
   * Lead pentru butonul WhatsApp de pe mobil.
   */
  const trackMobileWhatsapp = () => {
    if (typeof window !== "undefined" && (window as any).fbq) {
      (window as any).fbq("track", "Lead", {
        content_name: displayName,
        content_category:
          c.category || c.caroserie || "Auto",
        value: car.price ?? c.pret ?? 0,
        currency: "EUR",
        lead_type: "WhatsApp_Mobile_Bottom",
      });
          ...
          <div className="grid gap-8 lg:grid-cols-12 lg:gap-10">
            ...
            <div className="lg:col-span-5">
              <div className="lg:sticky lg:top-24">
                ...
                <h1 className="font-[family-name:var(--font-syne)] text-[28px] font-bold leading-tight text-[#111111] lg:text-4xl">
                  {title}
                </h1>
          ...
          <div className="mt-8 space-y-12 lg:mt-10">
            {specs.length > 0 && (
              <section>
                ...
                <div className="mt-6 grid grid-cols-1 gap-y-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                  {specs.map((spec) => (
                    <SpecCell
                      key={spec.label}
                      icon={spec.icon}
